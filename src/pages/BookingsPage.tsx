...
function weatherEmoji(code: number): { icon: React.ReactNode; label: string; emoji: string } {
  if (code === 0)
    return { icon: <FaSun className="text-yellow-300" />, label: 'Clear', emoji: '☀️' };
  if (code <= 3)
    return { icon: <FaCloud className="text-gray-400" />, label: 'Cloudy', emoji: '⛅' };
  if (code <= 48)
    return { icon: <FaSmog className="text-gray-400" />, label: 'Foggy', emoji: '🌫️' };
  if (code <= 67)
    return { icon: <FaCloudRain className="text-blue-400" />, label: 'Rain', emoji: '🌧️' };
  if (code <= 77)
    return { icon: <FaSnowflake className="text-blue-200" />, label: 'Snow', emoji: '🌨️' };
  if (code <= 82)
    return { icon: <FaCloudRain className="text-blue-400" />, label: 'Showers', emoji: '🌦️' };
...
export default function BookingsPage() {
  ...
  const handleShareMonth = async (month: string, monthBookings: Booking[]) => {
    ...
    try {
      ...
      const file = new File([blob], `${month.replace(' ', '-')}-schedule.png`, {
        type: 'image/png',
      });
          ...
          {(['upcoming', 'past'] as const).map((t) => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg font-bold text-sm transition-all ${
                tab === t ? 'bg-yellow-500 text-slate-900 shadow' : 'text-gray-400 hover:text-white'
              }`}
            >
              {t === 'past' ? (
                ...
                {grouped[month].map((b, i) => {
                  ...
                  return (
                    ...
                    >
                      {/* Accent strip */}
                      <div
                        className={`absolute left-0 top-0 bottom-0 w-1 ${isNight ? 'bg-blue-500' : 'bg-yellow-500'}`}
                      />
                      ...
                      <div className="pl-5 pr-4 py-4 flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-2 flex-wrap">
                            <div
                              className={`flex items-center gap-1.5 text-xs font-bold ${isNight ? 'text-blue-300' : 'text-yellow-300'}`}
                            >
