...
import { Trophy, Clock, CheckCircle2, AlertCircle, ChevronDown } from 'lucide-react';
...
function FlagImg({
  ...
  const cls = size === 'sm' ? 'w-8 h-5 object-cover rounded' : 'w-12 h-8 object-cover rounded';
  return (
    <img
      src={src}
      alt={alt}
...
function MatchCard({
  ...
  // Pre-fill existing prediction for selected player
  useEffect(() => {
    if (!selectedPlayer) {
  ...
  return (
    <div className="bg-slate-800 border border-slate-700/60 rounded-2xl overflow-hidden shadow-xl">
      ...
      <div className="px-6 py-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            {match.result === 'draw' && (
              <span className="text-xs bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-full font-semibold mt-1">
                Draw
              </span>
            )}
          </div>
      ...
      {/* Predictions list - visible after lock */}
      {(isLocked || match.status === 'completed') && match.predictions.length > 0 && (
  ...
  return (
    <div className="min-h-screen bg-slate-900 pb-16">
      ...
      <div className="container mx-auto max-w-2xl px-4 pt-6 space-y-6">
        {/* Player selector - global, applies to all match cards */}
        {matches.length > 0 && (
            ...
            <div className="divide-y divide-slate-700/40">
              {leaderboard.map((entry, i) => (
                <div key={entry.player_id} className="flex items-center gap-3 px-5 py-3">
                  ...
                  <span className="text-white text-sm font-medium flex-1">{entry.player_name}</span>
